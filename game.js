let score =JSON.parse(localStorage.getItem('score'))|| {
  ...
  loses : 0,
  same : 0,
}; /*this works the same as if we wrote this 
...
it is: if the score is true it will take the left side but if it is not then it take the right side
*/


/*
-JSON.Stringify(name of the subject): it is a Stringify function inside the JSON object that returns a string value --> we use it if a statment need an object but it can only support strings exp:localStorage.setItem('a name for this permenent variable','it value');
--localStorage.setItem('a name for this permanent variable','it value');: it is a setItem function inside the localStorage object that save a variable permenently so even if ze refrech the page we zont start from 0 again
-JSON.parse(a string object); : it is the revers of JSON.Stringify that retakes the object to the original js state --> we use it if we have an java script object notion (string) and we wants it real java script values exp:localStorage.getItem(object name) but in its JSON string form so ze use the reverse JSON parse.
*/
